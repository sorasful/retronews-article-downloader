...
def reconstruct_article_image(page_to_reconstruct:int, directory_holding_images: str):
    ...
    images_paths_for_page = sorted([str(x) for x in filter(lambda x: re.search(f"\d+-\d+-{page_to_reconstruct}-\d+-\d+.jpg", x.name), directory_holding_images.iterdir())])

    # sort the name of the images, get the last one (with biggest row and col)
    # then use regex to capture the values
    max_col, max_row = map(int, re.search(f"\d+-\d+/\d+-\d+-{page_to_reconstruct}-(\d+)-(\d+).jpg", images_paths_for_page[-1]).groups())
